...
/* Properties for end of calibration text */
...
/* Properties of the "Begin Experiment" button */
.end_cali01 {position: absolute;
	left: 16.3cm;
	top: 8cm;
	padding: 8px;}
	
/* Text when calibration trial timedout */
caliout_text{  position: absolute;
    left: 13cm;
    top: 9cm;
	font-size: 1cm;
}
...
<!-- Correc/Incorrect Text & Pics: -->
<p id="incorrect" style="display:none">Incorrect</p>
<p id="correct" style="display:none">Correct</p>
<img id="correct_pic" src="https://i.ibb.co/nQdmff5/correct.gif" style="width:150px;height:150px;position:absolute;left:18cm;top:6cm;display:none">
<img id="incorrect_pic" src="https://i.ibb.co/z5L7FmJ/incorrect.gif" style="width:150px;height:150px;position:absolute;left:17.5cm;top:6cm;display:none">
...
<button id="introbutton" style="display:none" class="introbutton01" type="button" onclick="cali_trials()">Begin Calibration</button>
<img id="introimg1" src="https://i.ibb.co/PxM3KCK/m-nut.jpg" style="width:100px;height:100px;position:absolute;left:16cm;top:15.7cm;display:none">
<img id="introimg2" src="https://i.ibb.co/7Nss3jc/n-nut.jpg" style="width:100;height:100px;position:absolute;left:21cm;top:15.5cm;display:none">
<img id="looking_glass_intro" src="https://i.ibb.co/RBCXC2s/looking-glass.gif" style="width:110px;height:110px;position:absolute;left:18.8cm;top:8.3cm;display:block">
...
<p002b2l2 id="introtext32" style="display:none">On the next set of trials your task is identical to the previous set of trials you completed. HOWEVER, now your task is to find the hidden acorn and give a response as quickly as possible on each trial. The upcoming second block of speed trials will be signified by the below alarm clock and at the end of each trial you will be shown how seconds you took searching for the acorn!</p002b2l2>
<p002b2l3 id="introtext42" style="display:none">When you are ready, press the button below to begin the next block.</p002b2l3>
<img id="alarm_intro" src="https://i.ibb.co/mDP0gy4/alarm.gif" style="width:150px;height:150px;position:absolute;left:17.6cm;top:6cm;display:none">

<!-- End of calibration block text: -->
<c1 id="cali_end_1" style="display:none"> You have now finished the calibration trials! When you ready, press the button below to begin the first block of the experiment.</c1>
<button id="cali_end_button" style="display:none" class="end_cali01" type="button" onclick="introbut()">Begin the First Experimental Block</button>

<!-- Text For When Timed Out Of Calibration Trials: -->
<caliout_text id="cali_out"></caliout_text>

<!-- End Of Expt Text: -->
<p id="endexpt" style="display:none">Finshed!</p>

<!-- Trial Timer and Alarm and Looking Glass: -->
<timer id="trial_timer" style="display:block"></timer>

<img id="alarm" src="https://i.ibb.co/mDP0gy4/alarm.gif" style="width:150px;height:150px;position:absolute;left:17cm;top:12cm;display:none">
<img id="looking_glass" src="https://i.ibb.co/RBCXC2s/looking-glass.gif" style="width:130px;height:130px;position:absolute;left:17cm;top:12cm;display:none">
...
var RTs_avg4 = [];
var RTs_avg5 = [];
var timer = 0; // Trial time tracker!
var ready = 0; // When to begin the experimental tirals
var cali_expt = 1; // Calibration trials are active
var cali_condition = [1,2]; // The single patch trial condition type for calibration tirals
var all_pics = document.getElementsByClassName("classy"); // Define variable to controll "classy" class
var opa_level = 1; // Determine the initial calibration trial opacity level
var cali_rep = 999; // Default value for the initial calibration trial response so first trial has opacity at 1
var cali_responses = []; // Variable to record all calibration trial responses
var timedout_opa = 0; // Initialise variable to increase opacity if Ps take  too long to respond with 0 - yet undetermined!
...
var st_p; // Variable to initialise start timer for practice trials
var et_p; // Variable to initialise end timer for practice trials
...
function beginTrialS() {
    ...
    document.getElementById('correct_pic').style.display='none';
    document.getElementById('incorrect_pic').style.display='none';
    document.getElementById('bTS').style.display='block';
    document.getElementById('alarm').style.display='none'; // Hide alarm
    ...
    // Stop showing timer and clear it:
    document.getElementById('trial_timer').style.display='none';
...
function block_2_start(){
  ...
  document.getElementById('alarm_intro').style.display='block';
...
let saveFile = () => {
    ...
    let data = 
        'Response RTs Condition Patch-Order Opacity-Level:' + '\r\n ' + 
				responses + '\r\n' +
        RTs + '\r\n' +
        CONDITIONS + '\r\n' +
        patch_order + '\r\n' +
...
function showTrial(){
  ...
  document.getElementById('looking_glass_intro').style.display='none';
  //document.getElementById('trial_timer').style.display='block';
  ...
  // Call the trial timer here:
  if (s_b == 1){
    document.getElementById('alarm').style.display='block'; // Show alarm
    showtime = setInterval(myTimer, 1000); // Update the timer every second!
  } else if (s_b == 0) {
    document.getElementById('looking_glass').style.display='block'; // Show alarm
  }
}
...
function Presbutton() { 
  ...
  document.getElementById('patch_1').style.display='none';
  document.getElementById('alarm').style.display='none'; // Hide alarm
  document.getElementById('looking_glass').style.display='none'; // Hide looking glass
  document.getElementById('cali_out').style.display='none';
  clearTimeout(timedout_cali); // Stop calling the timed out function!
  
  //document.getElementById("demo").innerHTML = con_num;
  
  // Show amount of seconds taken:
  if (s_b == 1){
    document.getElementById('trial_timer').innerHTML = "You spent " + timer + "s" + " searching for the acorn!"; // Define the timer and where it shows
    document.getElementById('trial_timer').style.display='block';
  }
  ...
  if (con_num == 1){
    ...
    document.getElementById('correct_pic').style.display='block';
    if (cali_expt == 0){
      responses.push(1);
    }
    cali_responses.push(1);
    cali_rep = 1; // Store calibration trial response
  } else if (con_num == 2) {
    document.getElementById('incorrect').style.display='block';
    document.getElementById('incorrect_pic').style.display='block';
    if (cali_expt == 0){
      responses.push(0);
    }
    cali_responses.push(0);
    cali_rep = 0; // Store calibration trial response
  }
  
  // Determine when to stop calibration trials:
  last_p = cali_responses.length; // Find the last array array entry 
  last_three = cali_responses.slice(last_p-3,last_p); // Determine last three calibration trial
  last_three = last_three.toString();
  if (last_three == "0,1,0"){
    ready = 1; // Signal to end calibration trials
  } 
  
  // What to do if calibration trials are active or not:
  if (cali_expt == 0) { // Calibration trials are NOT active
    ...
    if (trial_number == end_the_experiment){
      ...
      if (s_b == 0){
        ...
        setTimeout(block_2_start, 2000); // Call function after some defined time
      } else if (s_b == 1) {
        setTimeout(endExperiment, 2000); // Call function after some defined time
      }
    } else {
      ...
      CONDITIONS.push(con_num); // Record the trial condition
      RTs.push("BREAK"); // Split up RTs for different trials
      setTimeout(beginTrialS, 2000); // setTimeout reshows stimuli after 1 second
    }
    } else if (cali_expt == 1) { // Calibration trials ARE active
      if (ready == 0){
          setTimeout(cali_trials, 2000); // Call function after some defined time
        } else if (ready == 1) {
          document.getElementById('correct').style.display='none';
          document.getElementById('correct_pic').style.display='none';
          document.getElementById('incorrect').style.display='none';
          ...
          ///////////////////////////////////////////////////////////////////////////
          // Variable that determines opacity level:
          if (cali_rep == 1){
          opa_level = opa_level - 0.1; // Decrease by a little (0.1) if correct
          } else if (cali_rep == 0) {
            opa_level = opa_level + 0.1; // Increase by a little (0.1) if incorrect
          } else if (cali_rep == 999){
            opa_level = opa_level;
          }
    
          // FUNCTION TO CONTROL TRIAL OPACITY:
          for (i = 0; i < all_pics.length; i++) {
            all_pics[i].style.opacity = opa_level;
          }
          ///////////////////////////////////////////////////////////////////////////
          
          document.getElementById('header').style.display='block';
          document.getElementById('cali_end_1').style.display='block';
          document.getElementById('cali_end_button').style.display='block';
          s_b = 0; // Initiate block 1 trials
          cali_expt = 0; // Start recording responses and time etc
      }
    }
}
...
function Abbutton() { 
  ...
  if (con_num == 2){
    ...
    cali_responses.push(1);
    cali_rep = 1; // Store calibration trial response
    // Store calibration trial response
  } else if (con_num == 1) {
    document.getElementById('incorrect').style.display='block';
    document.getElementById('incorrect_pic').style.display='block';
    if (cali_expt == 0){
...
function Abutton() {
  ...
  if (patch_number == 2) {
    ...
    if (trial_number == end_the_experiment){
      ...
      CONDITIONS.push(conM_num); // Record the trial condition
      RTs.push("BREAK"); // Split up RTs for different trials
      setTimeout(beginTrialS, 2000); // Call function after some defined time
    }
  } else if (patch_number == 3) {
...
// Function to determine what happens when calibration trial is timedout
function cali_timeout() {
  document.getElementById('pic5').style.display='none';
	document.getElementById('button001').style.display='none';
  document.getElementById('button002').style.display='none';
	document.getElementById('Atext').style.display='none';
	document.getElementById('bTS').style.display='none';
  document.getElementById('patch_1').style.display='none';
  document.getElementById('cali_out').style.display='block';
  document.getElementById("cali_out").innerHTML = "You took too long to respond!";
  
  // Increase opacity if Ps take too long to respond:
  cali_rep = 0;
  cali_responses.push(0); // Record an incorrect response
  ...
  // Return to calibration trials after 2 seconds:
  clearTimeout(timedout_cali); // Stop calling the timed out function!
  if (ready == 0) {
    setTimeout(cali_trials, 2000);
  } else if (ready == 1){
    document.getElementById('correct').style.display='none';
    document.getElementById('correct_pic').style.display='none';
    document.getElementById('incorrect').style.display='none';
    ...
    document.getElementById('cali_end_button').style.display='block';
    s_b = 0; // Initiate block 1 trials
    cali_expt = 0; // Now signal the end of calibration trials
  }
}

// Calibration single patch trials:
function cali_trials() {
  ...
  // Make sure that no real trials are triggered:
  s_b = 2;
  ...
  // Shuffle then determine what sinlg epatch condition we are on:
  shuffle(cali_condition);
  con_num = cali_condition[0];
  
  // Make the images, image-labels and buttons visible after pressing the "Begin Calibrations" button:
  ...
  document.getElementById('cali_out').style.display='none';
	...
	document.getElementById("patch_1").onmouseover = function() {
		mouseOver5(); // Call function to determine what happens when mouse is over image 
		st_p = new Date(); // Define start timer as variable
		starttime_p = st_p.getTime(); // Start timer when image is uncovered
		};
	document.getElementById("patch_1").onmouseout = function() {
		mouseOut5(); // Call function to determine what happens when omuse is not over image
		et_p = new Date(); // Define end timer as variable
		endtime_p = et_p.getTime(); // Determine when the cursor is no longer over image
    rt_p = (endtime_p - starttime_p)/1000; // Calcuate rt for pic1:
		};
    
  // Time out after 3 seconds  
  timedout_cali = setTimeout(cali_timeout, 3000);
}


// Determine what trials to show in the FIRST block: 
function introbut() { // Single patch trials
  document.getElementById('alarm_intro').style.display='none';
  //patch_number = patch_number_pre[trial_number]; // Determine what patch type we are looking at
  ...
  if (patch_number == 1){ // 1 patch trials:
    ...
    document.getElementById('introimg2').style.display='none';
    document.getElementById('introbutton').style.display='none';
    document.getElementById('cali_end_1').style.display='none';
    document.getElementById('cali_end_button').style.display='none'
